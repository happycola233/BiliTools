...
歌词说明浮层、下载任务菜单与批量管理面板的 Material 表面共用 `floatingPanelContainerColor`：浅色使用 `cardContainerColor`（T98 近白），通过投影与页面分离，避免大面积 T90 模态底显灰；深色与纯黑模式使用 `modalContainerColor`，下载面板继续保留对话框描边。两种模式的背景都完全不透明，并跟随主题配色。
