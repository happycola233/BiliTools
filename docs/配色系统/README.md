...
三个脚本都依赖 `material-color-utilities-python` 与 `pillow`：

```bash
pip install material-color-utilities-python pillow
```

> Windows 上该库有个 bug：它会 `import curses`，而 Windows 没有这个模块。三个脚本开头都用一段 stub 顶掉了，不要删。
...
| | 取哪个角色 | 浅色 | 深色 | 用在哪 |
|---|---|---|---|---|
| **填充面** | `primaryFixedDim` | 同一个值 | 同一个值 | 主按钮底、选中胶囊、勾选框、开关轨道、滑条已选段、悬浮按钮、普通底栏选中气泡 |
| **压在填充面上的内容** | `onPrimaryFixed` | 同一个值 | 同一个值 | 上述色块之上的文字、对勾、图标 |
| **前景色** | `primary` | T36（深） | T84（浅） | 正文链接、强调文字、强调图标、进度条、液态底栏选中项 |
...
M3 的 **fixed 色组**正是为这种场景设计的：它的定义就是「不随明暗模式变化」。把色块类角色全部迁到 fixed 色组之后，设置页的色块所见即所得，两个模式的按钮也长得一样。

### 什么时候不能用填充色

填充色对页面底的对比度只有 1.17~1.52:1（浅色模式），远达不到非文字元素 3:1 的门槛。能不能用它，取决于**色块之上有没有能独立承担状态表达的深色元素**：

- 按钮、胶囊、勾选框、开关、滑条 —— 上面压着文字/对勾/滑块，状态一眼可辨，**用填充色**；
...
`FloatingActionButton` 这类元素浮在页面底之上，而深浅同值的浅填充色恰好在两个模式下都能浮起来：
...
`secondaryContainer` 在深色模式是暗色（T34），拿它当悬浮按钮底会直接糊进深色页面里，`ParseScreenContent` 的快捷下载按钮原来就踩了这个坑。
...
| 次级操作 | `secondaryContainer` | C22 | 复制字幕、批量面板里的清除记录 |
| 中性 | `surfaceContainerHigh` | C7 | 未选中的分段按钮 |
...
**开关的滑块要按模式取色。** 轨道两个模式都用填充色，但滑块不行：浅色模式取白色（这是开关这个控件固有的样子），深色模式必须反过来取 `onFill` 的深色——白滑块在深色模式会连同周围的浅轨道糊成一整块亮斑。滑条的滑块两个模式都取深色，因为滑条两条轨道都很细，白滑块会连同近白的卡片底一起糊掉，读不出当前值。
...
| 悬浮按钮 / 页面底 | 深色 3:1 | 1.20:1（靠投影） | 9.07:1 |
| 卡片对页面 / 对内嵌 | 1.09 | 1.099 / 1.129 | 1.408 / 1.241 |
| 填充色最大通道 | 220 ~ 252 | 225 ~ 250 | 同浅色 |

浅色模式下填充面对页面底达不到 3:1 是**设计取舍**，不是缺陷：淡雅浅色风格要求色块本身很淡，状态表达交给压在上面的深色内容与投影。详见第二节。
