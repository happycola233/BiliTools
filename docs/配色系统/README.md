...
色相之间间距 25~35 度，绕满一圈。樱粉是**默认方案**：用户从未选择过手动配色时，关掉动态取色会回落到樱粉；选择过配色后则恢复上次的选择。粉色是 B 站的代表色，但刻意没去对齐品牌色 `#FB7299`（那个饱和度放在淡雅浅色风格里太跳），而是取了同色系里淡雅的一档。
...
浅色模式下，快捷下载按钮与下载页操作菜单的主按钮使用 `secondaryContainer`，和“复制字幕”等次级操作保持同一强调层级。下载页菜单展开后的操作项带有明确文案，单独使用更轻的 `primaryFixed` + `onPrimaryFixed`；它与首页主操作按钮保持同色系，但不会形成同样重的色块，主菜单按钮与子项也可以分别取色。深色模式下 `secondaryContainer` 与 `primaryFixed` 都容易糊进页面底，因此悬浮主按钮和展开项均切回深浅同值的 `primaryFixedDim`：
