...
/**
 * 各形态的内置默认模板。形态之间互不继承，改一个不会牵动别的。
 * 默认值必须能扛住变量为空的情况：凡是「某些形态才有」的变量，
 * 一律包进可选片段，避免留下 `(P)`、`EP` 这类残渣。
 */
object NamingTemplates {
    ...
    fun default(shape: NamingShape, scope: NamingTemplateScope): String =
        defaults.getValue(shape)[scope].orEmpty()

    fun resolve(
        overrides: Map<NamingShape, NamingTemplateSet>,
        shape: NamingShape,
        scope: NamingTemplateScope,
    ): String = overrides[shape]?.get(scope) ?: default(shape, scope)

    fun sourceOf(
        overrides: Map<NamingShape, NamingTemplateSet>,
        shape: NamingShape,
        scope: NamingTemplateScope,
    ): NamingTemplateSource = if (overrides[shape]?.get(scope) != null) {
